...
contract L1Token is IArbTokenL1, ICustomToken, ERC20, Ownable {
    ...
    constructor(
        address _bridge,
        address _router
    ) public ERC20("TEST_HGL", "TEST_HGL") {
        bridge = _bridge;
        router = _router;
     ...
     * @notice should increase token supply by amount, and should (probably) only be callable by the L1 bridge.
     */
    function bridgeMint(address account, uint256 amount) external override onlyL1Gateway {
     ...
     * @notice should decrease token supply by amount, and should (probably) only be callable by the L1 bridge.
     */
    function bridgeBurn(address account, uint256 amount) external override onlyL1Gateway {
