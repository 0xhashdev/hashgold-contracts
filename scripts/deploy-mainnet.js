...
const l1Provider = new providers.JsonRpcProvider(process.env.MAINNET_L1RPC)
...
const main = async () => {
   ...
   */
  const L1MainnetHashgold = await (
    await ethers.getContractFactory('L1MainnetHashgold')
  ).connect(l1Wallet)
   ...
   */
  const L2MainnetHashgold = await (
    await ethers.getContractFactory('L2MainnetHashgold')
  ).connect(l2Wallet)
  ...
  const l2Token = await L2MainnetHashgold.deploy(
    l2Gateway,
    l1Token.address
  )
